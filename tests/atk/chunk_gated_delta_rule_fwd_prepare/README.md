...
`atk_chunk_gated_delta_rule_fwd_prepare.json` 内置 200 条（100 shape × bf16 双 seed；fp16 入口已改回 bf16）。全部 `use_qk_l2norm=True`、`use_exp2=True`、`use_gate=False`、`use_beta_sigmoid=True`、`allow_neg_eigval=True`，覆盖 GVA、尾块 `T%64!=0`、不满 pack、`V=128/256`、packed varlen（`B=1` + `seqlens`）。

`atk_chunk_gated_delta_rule_fwd_prepare_perf.json` 仍为定长 `T>=256` 的前 6 条（不含 `seqlens`），与变长补齐前一致。
